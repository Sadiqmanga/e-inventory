...
$result1 = $mysqli->query(
    ...
    if (isset($_POST['update-transaction'])) {
        ...
        $qry2 = "SELECT SUM(items_avail) AS total_stock FROM pi_tbl_stock WHERE product_id=$product_id";
        $result2 = $mysqli->query($qry2) or die($mysqli->connect_error); 
        $row2 = $result2->fetch_assoc();
        $row2TotalStock = $row2["total_stock"];

        $qry3 = "SELECT SUM(qty) AS total_cart FROM pi_tbl_carts WHERE product_id=$product_id AND invoice_no!=$invoice_ref";
        ...
        if ($qty <= $availableStock) {
            // $qry4 = "UPDATE pi_tbl_carts SET qty=$qty, amount='$updatedAmount' WHERE product_id=$product_id AND invoice_no='$invoice_no'";
            if ($mysqli->query("UPDATE pi_tbl_carts SET price=$price, qty=$qty, amount=$updatedAmount WHERE id=$id")) {
    ...
    if (isset($_POST['approve-transaction'])) {
        ...
        $sql1 = "SELECT * FROM pi_tbl_transactions WHERE invoice_no = ?";
        ...
        if (empty($trnError)) {
            $sql5 = "INSERT INTO pi_tbl_transactions(saler_id,user_id,invoice_no,payment_mode,total_amount,discount) VALUES(?, ?, ?, ?, ?, ?)";
            $stmt = $mysqli->prepare($sql5);
            $stmt->bind_param("ssssss", $saler_id, $user_id, $invoice_no, $payment_mode, $total_amount,$discount);
            if($stmt->execute()) {
                $mysqli->query("UPDATE pi_tbl_carts SET status='Approved' WHERE invoice_no=$invoice_no");
                echo 
                    "<script>
                        (function() {
                            window.addEventListener('load', function() {
							...
							<div class="col-lg-9 ml-auto mr-auto">
								<div class="card">
									<div class="card-body">
                                        <?php
                                            $result = $mysqli->query(
                                                "SELECT a.id, a.product_id, a.invoice_no, a.product_name, a.product_cat, a.price, a.qty, a.amount, a.status, a.cart_date
                                                FROM pi_tbl_carts AS a
                                                WHERE a.invoice_no=$invoice_ref;") or die($mysqli->connect_error); ?>
